...
/// ```
pub fn generate_auth_code<T>(shared_secret: T, time_offset: Option<i64>) -> Result<String, Error>
where
    T: AsRef<[u8]>,
{
...
fn generate_auth_code_for_time<T>(shared_secret: T, timestamp: i64) -> Result<String, Error>
where
    T: AsRef<[u8]>,
{
    let mut full_code = {
        ...
        let full_code_slice: [u8; 4] = slice
            .try_into()
...
fn get_hmac_msg<T>(secret: T, bytes: &[u8]) -> Result<HmacSha1, Error>
where
    T: AsRef<[u8]>,
{
    let decoded = base64::engine::general_purpose::STANDARD.decode(secret)?;
    let mut mac = HmacSha1::new_from_slice(&decoded[..]).map_err(|_e| Error::EmptySecret)?;
...
mod tests {
    ...
    fn generates_confirmation_key_for_time() {
        let identity_secret = "000000000000000000000000000=";
        let hash =
            generate_confirmation_key_for_time(identity_secret, Tag::Allow, 1634603498 as i64)
                .unwrap();
